...
public class POSLib {
     ...
     */
        public static ArrayList<Map<String, Map<String, Double>>> loadData(String filename) throws Exception {
        if (debug) System.out.println("loading data from "+filename);
        BufferedReader in = new BufferedReader(new FileReader(filename));
        String line;

        Map<String, Map<String, Double>> obsMap = new HashMap<>();
        Map<String, Map<String, Double>> transMap = new HashMap<>();
        ArrayList<Map<String, Map<String, Double>>> mapList = new ArrayList<>();
...
//                        System.out.println("observation "+state+" "+word+" "+score);
                        innerMap.put(word, score);
                        obsMap.put(state, innerMap);
                    }
                    else {
                        String next = parts[i];
//                        System.out.println("transition "+state+" "+next+" "+score);
                        innerMap.put(next, score);
                        transMap.put(state, innerMap);
                    }
                }
            }
        }
